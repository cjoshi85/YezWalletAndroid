 ...
 */
export async function encryptWIF(wif, passphrase) {
...
//   return encryptedKey
    ...
    if (account) {
        ...
        return c.then((result) => {
            var derived = Buffer.from(result.toString('hex'))
           // var derived = result.toString('hex')
            const derived1 = derived.slice(0, 64)
            const derived2 = derived.slice(64)
            // AES Encrypt
            const xor = XOR(account.privateKey.slice(0,32), derived1)
 ...
 */
export async function decryptWIF(encryptedWIF, passphrase, scrypt_params = SCRYPT_PARAMS) {
...
export async function generateEncryptedWIF(passphrase, existingWIF) {
    ...
    try{
        ...
        for (let i = 0; i < privatekey.length; i++) {
            let str = privatekey[i].toString(16)
            str = str.length === 0 ? '00'
                : str.length === 1 ? '0' + str
                    : str
            key += str
        ...
        const account = await new wallet.Account(existingWIF || key)
